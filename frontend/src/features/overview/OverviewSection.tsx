...
/**
 * Overview section — the smart command center.
 * Surfaces alerts, condition summary, recommendations, patient data, and backend status.
 */
export function OverviewSection() {
  ...
  }, [intelligenceReportLoaded, patientOverviewLoaded, userId, refetchIntelligenceReport, refetchPatientOverview]);
  ...
  const activeAlerts = report?.alerts.filter((a) => !dismissedAlerts.has(a.id)) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Condition summary + Backend status */}
          ...
          <div className="lg:col-span-2">
            <GlassCard padding="md" className="h-full">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-sm font-semibold theme-text">Welcome to CKD Guardian</h2>
              </div>
              <p className="text-xs theme-text-muted leading-relaxed">
                Start by submitting a prediction from the <span className="text-sky-400 font-medium">Data Input</span> tab.
                Your personalized health insights, alerts, and recommendations will appear here.
              </p>
            </GlassCard>
          </div>
          <BackendStatus />
      ...
      {/* Patient data (mock overview) */}
      {!data && !loading ? (
        <GlassCard padding="md" className="text-center py-12">
          <p className="text-sm text-red-400 mb-3">Failed to load patient data</p>
          <button
